...
A 3D line segment of 76.9 meter is defined and serve as the true values in the experiments in this subsection. The endpoints of this 3D line segment locate on the road surface in the test area (German highway A9) with 7 aerial images coverage. In the form of a set of 3D points, this 3D line segment is generated by linear interpolating several points with 0.2 meter spaces (considering DSM grid of 0.2 meter) between its two endpoints.
...
The initial estimates for non-linear LS adjustment is generated by projecting the true line segment onto the DSM and additionally adding some known biases in the position of its endpoints.
\begin{table} [h!]
  \centering
  ...
  \bottomrule
  \end{tabular}
  \caption{The added biases in the endpoints of the DSM profile segment projected from the true line segment. This biased line segment serves as the initial approximation in non-linear LS adjustment.}
...
\cref{fig:Simu3D_2} shows the reconstructed and the true line segments in UTM coordinate system (in Zone 32 ??). The distances from the reconstructed line segment to the true one with 20 centimeter spacing are plotted into histogram in \cref{fig:SimuHist_2}.
...
the differences appears non-normal distributed and the first moment is not zero (proved by statistical test)
That 
