...
The initial estimates for non-linear LS adjustment is generated by projecting the true line segment onto the DSM.% and additionally adding some known biases in the coordinates of the true line segment's endpoints.
%\begin{table} [h!]
%  \centering
...
%  \bottomrule
%  \end{tabular}
%  \caption{The added biases in the object coordinates of the true line segment's endpoints, i.e. the known biases that are added in the unknown parameters in the LS adjustment.}
...
\cref{fig:Simu3D_2} shows the reconstructed and the true line segments in UTM coordinate system (in Zone 32N). %The distances from the reconstructed line segment to the true one are plotted into histogram in \cref{fig:SimuHist_2}.
The distances from the reconstructed line nodes to the true line segment are collected, resulting in sample size of $9$. The sample mean is $-0.004$ [meter] and the sample standard deviation is $0.012$ [meter]. This sampling procedure is assumed to be independent and random.%??? (t test assumptions)

A two-tailed t-test is adopted to test if the reconstructed line segments are close enough to the true line segments, i.e. the population mean significantly equals zero. Null hypothesis ($H_0$) and (two-tailed) alternative hypothesis ($H_A$) are stated as:
\begin{equation*}
\begin{split}
H_0: \mu=0\\
H_A: \mu\neq0
...
With the sample mean $\overline{x}=-0.004$,
...
As the test statistic $T_{obs}=-1$ is neither less than $-T_{(0.95,8)}=-2.306$ nor greater than $T_{(0.95,8)}=2.306$, i.e. not in the rejection region, we fail to reject the null hypothesis. In other words, with 95\% confidence we can claim that the distances from the reconstructed line nodes to the true line segment significantly equals to $0$. This indicates that the non-linear LS adjustment model for 3D reconstruction is correct.


\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{Simu_3D_2.png} %%% 換，字重疊。
...
\cref{fig:Simu3D_1} shows the reconstructed line segments and the DSM profile in UTM coordinate system (in Zone 32N). The distances from the reconstructed line segment to the DSM profile are plotted into histogram in \cref{fig:SimuHist_1}. They are collected along the reconstructed line segments with 0.2 meter spacing (considering the DSM grid of 0.2 meter), resulting in sample size of $381$. The sample mean is $-0.029$ [meter] and the sample standard deviation is $0.083$ [meter]. This sampling procedure is assumed to be independent and random.%??? and normal distributed (Z test assumptions)

In order to prove that the population mean of the distances from the reconstructed line segments to the DSM profile is significantly non-zero, a two-tailed Z-test is adopted. Null hypothesis ($H_0$) and (two-tailed) alternative hypothesis ($H_A$) are stated as:
\begin{equation*}
\begin{split}
H_0: \mu=0\\
H_A: \mu\neq0
...
Z_{0.975}=1.96
\end{equation*}
leads to the decision rule: if $Z_{obs}$ is less than $-1.96$ or greater than $1.96$, reject the null hypothesis.
...
As the test statistic $Z_{obs}\approx-6.82$ is less than $-Z_{0.975}=-1.96$, i.e. in the rejection region, the null hypothesis is rejected. In other words, with 95\% confidence we can claim that the distances from the reconstructed line segment to the to the DSM profile is significantly non-zero. This tells that ... ???. %This phenomenon is as expected, as mentioned in sec:DSM??? 


\begin{figure}
  \centering
  \includegraphics[width=0.9\textwidth]{Simu_3D_1.png} %%% 換，字重疊。
  \caption{\small The reconstructed line and the unrefined DSM profile.}
  \label{fig:Simu3D_1}
  \vspace{1cm}
  \includegraphics[width=0.8\textwidth]{Simu_hist_1.png}
...
A continuous lane marking of 453.6 meters length is reconstructed. \cref{fig:Test3D_1} shows the reconstructed line segments and the DSM profile in UTM coordinate system (in Zone 32N). The distances from the reconstructed line segment to the DSM profile are plotted into histogram in \cref{fig:TestHist_1}. They are collected along the reconstructed line segments with 0.2 meter spacing (considering the DSM grid of 0.2 meter), resulting in sample size of $2246$. The sample mean is $-0.061$ [meter] and the sample standard deviation is $0.188$ [meter]. This sampling procedure is assumed to be independent and random.%??? and normal distributed (Z test assumptions)

To prove that the DSM height profile is significantly lower than the reconstructed line segments for more than $5$ centimeters, a lower-tailed Z-test is adopted. Null hypothesis ($H_0$) and (one-tailed) alternative hypothesis ($H_A$) are stated as:
\begin{equation*}
\begin{split}
H_0: \mu\geq-0.050\\
H_A: \mu<-0.050
...
With the sample mean $\overline{x}=-0.061$,
the proposed population mean $\mu_0=-0.050$,
the sample standard deviation $\sigma=0.188$,
and sample size $n=2246$, the test statistic for a One Sample Z Test has a calculated value:
\begin{equation*}
Z_{obs} = \frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}}=\frac{-0.061-(-0.050)}{0.188/\sqrt{2246}}\approx-2.77
\end{equation*}

As the test statistic $Z_{obs}\approx-2.77$ is less than $-Z_{0.95}=-1.64$, i.e. in the rejection region, the null hypothesis is rejected. In other words, with 95\% confidence we can claim that the DSM profile is in average, statistically and significantly lower than the reconstructed line segments for more than $5$ centimeters. %This phenomenon is as expected, as mentioned in sec:DSM??? 
