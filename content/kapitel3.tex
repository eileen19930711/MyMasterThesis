...
From \cref{fig:SimuSigmaxx} it can be seen that the estimated parameters in horizontal direction, i.e. $\hat{X}$ and $\hat{Y}$, have smaller priori variances values $\sqrt{\sigma_{\hat{X}}^2+\hat{\sigma}_{\hat{Y}}^2}$ with the increase of covering images. The priori variance $\sigma_{\hat{Z}}$ of the estimated parameter $\hat{Z}$ generally has this trend whereas some deviations are involved. 
