...
A significance level $\alpha=0.05$ is selected, which is $0.025$ on each tail of the population, i.e. the area in body is $0.975$ out of 100\%. The corresponding z-score:
\begin{equation*}
Z_{0.975}=1.96
\end{equation*}
...
\begin{equation*}
Z_{obs} = \frac{\overline{x}-\mu_0}{\sigma/\sqrt{n}}=\frac{-0.029-0}{0.083/\sqrt{381}}\approx6.82
\end{equation*}

As the test statistic $Z_{obs}\approx6.82$ is greater than $T_{(0.95,8)}=2.306$, i.e. in the rejection region, the null hypothesis is rejected. In other words, with 95\% confidence we can claim that the distances from the reconstructed line segment to the to the DSM profile is significantly non-zero. This tells that ... ???. %This phenomenon is as expected, as mentioned in sec:DSM??? 
