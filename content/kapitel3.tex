...
This section aims to verify the correctness %definition of correctness
of the derived LS model using simulation data. By checking if the iteration scheme converges to the correct solution given biased initial values of the unknowns, the non-linear LS model's ability of "refining" the approximate line segment is judged.
...
Note that the quality of camera parameters (eo, io, additional parameters) do not have influences during simulation of 3D reconstruction since the simulated measurements are produced with the same set of camera parameters from 3D to 2D.
...
observations, unknowns, redundancies, 
