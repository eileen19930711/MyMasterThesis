...
The convergence threshold for 2D image point projection on DSM (see \cref{sec:LineProjectionOnDSM}) is selected as 0.5 m, as the height precision of the used DSM is around \textcolor{red}{50 cm}.


%%% sliding window %%%%%%%%%%%%%%%%

The length of the sliding window should be decided base on the expected curvature of the targeted line. Additionally for the sake of robustness, regression model requires as much independent measurements as possible. In the cases of lane markings on motorways, the sliding window length was fixed to 16 m as a compromise between robustness and the minimized systematic errors arisen from straight-approximated curvature. The step size of sliding window was set to half of the sliding window length, i.e. 8 m.
