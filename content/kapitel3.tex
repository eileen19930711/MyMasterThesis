...
\cref{fig:SimuError} shows the mean and variance of the added random Gaussian noise and the adjusted residuals. Comparing this two samples by applying a two-tailed two-sample Z-test. Null hypothesis and alternative hypothesis ($H_A$) are stated as:
\begin{equation*}
\begin{split}
H_0: \mu_1-\mu_2=0\\
H_A: \mu1-\mu_2\neq0
...
With significance level $\alpha=0.05$, the z-score is 
\begin{equation*}
Z_{0.975}=1.96
\end{equation*}

As all the test statistics $Z_{obs}$ of each segment are not less than $-Z_{0.975}=-1.96$ or greater than $Z_{0.975}=1.96$, i.e. not in the rejection region, as shown in fig???, the null hypothesis could not be rejected. In other words, 
