...
The appearance of lane markings on German roads including line type, color and width is specified depending on the road type. Different line types of lane markings are shown in \cref{fig:LaneMarkingTypes} and their line widths are defined in \cref{tab:LaneMarkingWidths}. %[source: Wikipedia, https://de.wikipedia.org/wiki/Fahrbahnmarkierung#Eigenschaften]

Because of lane marking's appearance, the problem of lane marking detection can be treated as a line detection problem. We restrict the proposed framework to lane markings with single white lines (dashed or continuous) of 0.3 meter width. Other types like in restricted zone, double lines, parking areas, temporal yellow lines in construction sites, are excluded. % https://www.transchool.lee.army.mil/adso/documents/zeichen.pdf
...
The $\sigma$ value for Gaussian smoothing is set to be 1.8 % From HALCON Referenz: For the choice of the thresholds High and Low one has to keep in mind that the second directional derivative depends on the amplitude and width of the line as well as the choice of Sigma. Thus: you must set this value depending on the lane marking width?
in this work to slightly suppress the noise. The extracted lines of length less than 30 pixels % why
are rejected. Each accepted extracted line is rasterized as a set of points in image coordinates with sub-pixel precision.\cref{fig:LineExtraction} shows the extracted lines on the original image.
...
% Pre-warning line
\caption{\small Line types of lane markings [source Wikipedia].}
...
\begin{table}
    ...
    \bottomrule
    \end{tabular}
    \caption{\small Line widths of lane markings [source Wikipedia]}
...
\end{equation}
% To make it clearer, describe P,q,p more detailled
