...
For a later combination with point-wise extended collinearity equation \eqref{eq:Gfunction} in next section, we aim to fit the line equation in two-point form to the extracted lines produced in \cref{sec:LineExtraction} (in forms of sets of points in image coordinates). For such purpose, a non-linear mixed model is derived in \cref{subsec:NonLinear}. 
...
Since our aim is to reconstruct line segments taking into consideration of their position on all of the covering image, the adjustment will be done "line-wise"--- for a pair of $P_s$ and $P_e$, the measurements are collected correspondingly. Thus the subscription $l$ representing specific line segment will be left out in the followings.
...
At least two constraints are needed to fix rank deficiency???. There are three constraints used in this work:
\begin{itemize}
...
\subsection{Non-linear Mixed Model with constraints}
\label{subsec:LSadj}

By assuming a non-linear function being locally linear with small step size of the unknown quantities, a non-linear LS system becomes linear by changing the unknowns to their increasements.  through Taylor' polynomial, expansion, and is thus solvable

Function $\mathcal{H}$ actually do line fitting in all covering image spaces where the lines to be fitted come from a single line segment in object space. $\mathcal{H}^y$ is non-linear, thus we first have to do linearization.
...
Fixing the X, Y coordinates of the start-point would barely do harm to the result, regarding the fact that our DSM is noisy in Z-direction.
