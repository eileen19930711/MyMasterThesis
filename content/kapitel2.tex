...
\end{split}
\end{equation}
with $r=\sqrt{x_*^2+y^2}$, $x_*=\dfrac{x}{C_1}$ and radius\footnote{At the radius $R_0$ the radial symmetric distortion is zero by definition, which avoids too high distortion values at the edges and reduces the correlation with the focal length.} $R_0$ being set to 0.014m which corresponds to a third of the sensor diagonal.
...
% regarding the fact of problematic line x=const. (parallel to y-axis) and considering that the dataset (which will be described in more detail) observations in images are mainly in column direction 

In the case of simple linear regression as presented in \cref{subsec:LinearRegression}, the independent variable $x$ is error free, inconsistencies are only for the dependent variable $y$. Geometrically it means that the vertical distances from observed data to the fitted line is minimized. To minimize the 
%sum of squared 
...
In the case with inconsistencies $e_{x_i}$ and $e_{y_i}$ in both observations $x_i$ and $y_i$, the A-model 
% First mentioned here, you should explain what it is before.
with pseudo observation equations can be adopted and the observation equations are formulated:
