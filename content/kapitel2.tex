...
In \cref{subsec:LSwithConstraints} we firstly set-up a model which describes the fitting of a line to the measurements in all covering image space, and in which the fitting line is transformed from object space through collinearity equations. 

Regarding the fact that the collinearity is a point-wise condition, a line segment is represented by its two end-points whose object coordinates are the six unknowns in our LS model. 
Correspondingly, the observation equations in the LS model are line equations in two-point form. Such line equations has however a mathematical meaning of infinite length. Therefore, some constraints are necessary to avoid infinitely many solutions along a , arbitrary shift.
