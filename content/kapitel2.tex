...
The following sections introduce the principles of 3D lane marking reconstruction methods as well the materials used in this work, with the work flow shown in \cref{fig:FlowChart}.

\cref{sec:Materials} provides information about input dataset and some applied preprocessing steps. \cref{sec:LineExtraction} describes the applied standard line detection algorithm for labeling the lane markings.

To relate a point in object space 
\cref{sec:Geometry} introduces the imaging properties of aerial images and their mathematical models, including the collinearity equation and lens distortion correction.
...
% flowchart [!hb]
%% describe the workflow in the paragraphs, more details
\begin{figure}
  \centering
...
The images are acquired with a special flight configuration at both sides of the motorway which guarantees a continuous stereo view perpendicular to the lane marking direction.% The classical photogrammetric approach by flying in straight lines, which cover the whole motorway in a stereo view, would be possible, but would require more flight time and would produce many more images.
...
The \gls{eo} of the images are directly measured by a GNSS/Inertial system IGI IId.% Missing: specification oft he system, improvements by SAPOS correction,….
...
\end{itemize}
% conclusion: 所推為單片單點直接投影的精度；若為雙或多片前方交會則可提升精度
...
\clearpage
\paragraph{\gls{dsm}}
For each pair of stereo images, a disparity map is generated using \gls{sgm} algorithm. With disparity's property of being inversely proportional to depth, the disparity maps can be used to derive the DSM. In most of the cases, a point in object space is covered by more than two aerial images, resulting in more than one disparity map. The problem of ambiguities on height decision during DSM generations is solved by simply taking the median value derived from disparity maps in odd number of disparity maps cases, and the value just lower than the median in even number cases. %This results in systematic errors of having lower height value in some parts of DSM.(The whole proof of this sentence is more complicated, I hope you will be not asked about this.)
