...
\end{equation}
where\newline
the $N\times U$ design matrix is
\begin{equation*}
\mathsf{A}=\left.\dfrac{\partial\boldsymbol f(\boldsymbol x)}{\partial\boldsymbol x}\right|_{\boldsymbol x=\widehat{\boldsymbol x}^a}
\end{equation*}
the $U\times H$ constraint matrix is
\begin{equation*}
\mathsf{H}=\left.\dfrac{\partial\boldsymbol h(\boldsymbol x)}{\partial\boldsymbol x^T}\right|_{\boldsymbol x=\widehat{\boldsymbol x}^a}
\end{equation*}
...
With the assumption of equal-weighted observations, the task is to minimize the sum of residuals:
\begin{equation}
