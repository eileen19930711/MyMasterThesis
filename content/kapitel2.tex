...
In \cref{subsec:LSwithConstraints} we firstly set-up a model which transforms a line from object space to image space through collinearity equations, and this model in the same time describes the fitting of a line to the measurements on all covering images. Regarding the fact that the collinearity is a point-wise condition, a line segment is represented by its two end-points. The object coordinates of a pair of end-points are the six unknown quantities in our LS model. 
However, a line equation has a mathematical meaning of infinite length, as mentioned in \cref{sec:LineFitting}. Some constraints are necessary to somehow fix one of the end-points' position is necessary to avoid infinitely many solutions.
