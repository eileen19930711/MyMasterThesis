...
For all covering image $j=1,2,...J$, there are $2\times\displaystyle\sum_{j=1}^{J}N^j$ observation equations (being written in the structure of the Gauss-Markov model):
...
The constraint equations \eqref{eq:constraint1}, \eqref{eq:constraint2} and \eqref{eq:constraint3} can be stacked together in the vector form:
\begin{equation} \label{eq:convec}
\mathbf{h}(\hat{\mathbf{x}})=\mathbf{0}:\quad
\begin{bmatrix}
