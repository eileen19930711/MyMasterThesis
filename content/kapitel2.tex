...
%As a function can only have one output for each unique input
...
%, i.e. by switching and the line fitting model 
% that knowledge of X (using your original notation) tells us nothing about Y 
...
Simple linear regression is a linear regression model with a single explanatory variable. Given a data set $\{x_i,y_i\}^n_{i=1}$ of $n$ points on a 2D plane, a linear regression model assumes that the relationship between the dependent variable $y_i$ and the regressors $x_i$ is linear. This relationship is modeled through an error variable $e_{y_i}$--- an unobserved random variable that adds noise to the linear relationship between the dependent variable and regressors.
