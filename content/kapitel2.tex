...
A linear regression model describes a dependent variable as a linear function of the regressor (an independent variable). In the case when both dependent variable $x$ and regressor $y$ are measured with errors $e_x$ and $e_y$, a linear regression model takes the form:
...
Such errors-in-variables models, as the combination of \eqref{eq:MixModel1-1} and \eqref{eq:MixModel1-2}, account the measurement errors in both dependent variable and regressor. Further more, for the case of equal error variances, i.e. when $\delta=\dfrac{\sigma_{e_x}}{\sigma_{e_y}}=1$, it is a orthogonal regression model which minimizes the sum of squared perpendicular distances from the data points to the regression line.
...
which takes 2D coordinates of two points $\mathbf{p_s}(x_s,y_s)$ and $\mathbf{p_e}(x_e,y_e)$ that define an infinite line, and takes the measured x-coordinate $x$ of an image point $\mathbf{p}$, and returns the estimated image coordinates $\mathbf{\hat{p}}(\hat{x},\hat{y})$ which lies on the infinite line $\overline{\mathbf{p_s}\mathbf{p_s}}$.
...
\begin{split}
\mathbf{\hat{x}} = \mathcal{F}^x(\mathbf{p_s},\mathbf{p_e},x)\\
\mathbf{\hat{y}} = \mathcal{F}^y(\mathbf{p_s},\mathbf{p_e},x)
\end{split}
\end{equation}
